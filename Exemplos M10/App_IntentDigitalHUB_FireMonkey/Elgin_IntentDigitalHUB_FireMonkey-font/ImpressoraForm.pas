...
begin
  ...
  Impressora.ImprimeTexto(edtMensagem.Text,Alinhamento,Font,FontSize,cbCutPaper.IsChecked,cbSublinhado.IsChecked,cbNegrito.IsChecked, rbImpInterna.IsChecked, Retorno);
end;
...
begin
    ...
    Impressora.printerInternalImpStart(Retorno);
...
//      edtIP.Enabled := False;
  ...
  begin
    ...
    begin
      ...
      Impressora.PrinterExternalImpStart(modelo, config[0], tipoImp, strtoint(config[1]), Retorno);
      if (res <> 0) then
      begin
        res:= -1;
        type_impressora:= 'interna';
...
begin
  ...
  begin
    ...
    Impressora.PrinterExternalImpStart(modelo, conexao, tipoImp, 0, Retorno);
    if (res <> 0) then
    begin
      res:= -1;
      type_impressora:= 'interna';
...
//  TToastMessage.show(msg,3,40,TToastPosition.tpBottom);
  ...
  if (type_impressora = 'usb') or (type_impressora = 'externa') then
  begin
    res:= response;
...
begin
   Impressora.imprimeImagem(ImagePreVisualizacao.Bitmap,cbImageCutPaper.IsChecked, rbImpInterna.IsChecked, Retorno);
end;
...
begin
  ...
  if rbCBEsquerda.IsChecked then
   ...
   if not (barcodeType = QR_CODE) then
      Impressora.imprimeBarCode(barcodeType,edtCodigo.Text,Alinhamento,height,width, cbCodigoBarrasCutPaper.IsChecked, rbImpInterna.IsChecked, Retorno)
   else
      Impressora.imprimeQR_CODE(edtCodigo.Text,Alinhamento,width,cbCodigoBarrasCutPaper.IsChecked, rbImpInterna.IsChecked, Retorno);
end;
...
begin
   xml := 'xmlNFCe.xml';
   Impressora.ImprimeXMLNFCe(xml,'CODIGO-CSC-CONTRIBUINTE-36-CARACTERES',1,0,cbCutPaper.IsChecked, rbImpInterna.IsChecked, Retorno);
end;
...
begin
   xml := 'xmlSAT.xml';
   Impressora.ImprimeXMLSAT(xml,0,cbCutPaper.IsChecked, rbImpInterna.IsChecked, Retorno);
end;
